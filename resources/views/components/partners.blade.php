    ...
    <div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 sm:gap-6 place-items-center">
            ...
            @foreach($allPartners as $partner)
                <div class="group relative bg-white rounded-xl p-2 w-full max-w-[220px] aspect-square flex items-center justify-center shadow-sm hover:shadow-lg transition-all duration-300 transform hover:-translate-y-1">
                    <!-- Logo du partenaire -->
                    <img src="{{ asset('partenaires/' . Str::slug($partner) . '.png') }}" 
                         alt="Logo {{ $partner }}" 
                         class="relative h-24 w-auto object-contain transform transition-all duration-300 group-hover:scale-105 filter grayscale group-hover:grayscale-0">
                </div>
            @endforeach
        </div>
        ...
        <div class="mt-12 text-center">
            <div class="inline-flex flex-col items-center bg-white p-4 rounded-xl shadow-lg transform hover:-translate-y-1 transition-all duration-300">
